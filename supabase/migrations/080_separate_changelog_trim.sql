-- Migration: Run changelog trimming as its own call
-- Drafted: September 19, 2026
--
-- Measured through the API, run_data_quality_checks() took 5.6s with the 3,000-row
-- changelog trim inside it, against PostgREST's 8s statement limit; the checks
-- themselves take ~0.4s. A timeout would also roll back the trend update, so the
-- trim moves to its own function that the worker calls separately.

CREATE OR REPLACE FUNCTION public.trim_entity_changelog(p_limit int DEFAULT 3000) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE trimmed int;
BEGIN
  DELETE FROM entity_changelog WHERE id IN (
    SELECT id FROM entity_changelog WHERE created_at < now() - interval '90 days'
    ORDER BY created_at LIMIT least(greatest(p_limit, 0), 20000));
  GET DIAGNOSTICS trimmed = ROW_COUNT;
  RETURN jsonb_build_object('changelog_rows_trimmed', trimmed);
END;
$$;

REVOKE EXECUTE ON FUNCTION public.trim_entity_changelog(int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.trim_entity_changelog(int) TO service_role;

-- Review process without the trim (otherwise identical to 078)
