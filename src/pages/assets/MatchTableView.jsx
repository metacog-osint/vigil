  ...
  if (matches.length === 0) {
    return (
      <div className="cyber-card p-8 text-center">
        ...
        <p className="text-gray-500 text-sm mt-2">Your assets haven't been detected in threat intelligence yet.</p>
      </div>
    )
  }
      ...
      <table className="w-full">
        <thead className="bg-gray-900/50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">When</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-800">
          {matches.map(match => (
            ...
            >
              ...
              <td className="px-4 py-3">
                <span className={`px-2 py-1 rounded text-xs bg-${MATCH_TYPES[match.match_type]?.color || 'gray'}-500/20 text-${MATCH_TYPES[match.match_type]?.color || 'gray'}-400`}>
