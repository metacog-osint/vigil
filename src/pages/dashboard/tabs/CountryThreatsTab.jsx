  ...
  return (
    ...
    >
      <td>
        ...
        <div className="text-xs text-gray-500">{country.unique_actors || 0} actors</div>
      </td>
      <td>
      ...
      <td className="hidden lg:table-cell">
        <div className="flex flex-wrap gap-1">
          {country.attacking_countries?.slice(0, 2).map((c) => (
            <span key={c} className="px-1.5 py-0.5 text-xs rounded bg-gray-800 text-gray-400">
              {c}
            </span>
          ))}
          {country.attacking_countries?.length > 2 && (
            <span className="text-xs text-gray-500">+{country.attacking_countries.length - 2}</span>
          )}
        </div>
      </td>
    </tr>
    ...
    <div className="w-80 cyber-card">
      ...
      <div className="space-y-4 text-sm">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="cyber-card bg-gray-800/50">
            <div className="text-2xl font-bold text-yellow-400">{country.unique_actors || 0}</div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {country.targeted_industries.slice(0, 6).map((ind) => (
                <span key={ind} className="px-2 py-0.5 text-xs rounded bg-gray-800 text-gray-400">
                  {ind}
                </span>
              ))}
            ...
            <Geographies geography={GEO_URL}>
              {({ geographies }) =>
                geographies.map((geo) => {
                  ...
                  const countryData =
                    countryLookup[countryName] || countryLookup[geo.properties.ISO_A3]
