// Bulk IOC search page - search multiple indicators at once
...
export default function BulkSearch() {
  ...
  // Parse input into individual IOCs
  const parseInput = useCallback((text) => {
    // Split by newlines, commas, spaces, and common delimiters
    const items = text
      .split(/[\n,;\s]+/)
      .map((item) => item.trim())
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Group items by type for efficient searching
      const grouped = {
        cve: [],
        ioc: [],
      }
              ...
              <table className="cyber-table w-full">
                ...
                <tbody>
                  {results.map((result, index) => (
                    <tr
                      key={index}
                      className={result.found ? '' : 'opacity-60'}
                    >
                      <td>
                        {result.found ? (
