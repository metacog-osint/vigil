...
export default function ThreatHuntCard({
  ...
  const progressPercent = totalChecks > 0 ? Math.round((completedChecks.length / totalChecks) * 100) : 0
  const isComplete = progress?.status === 'completed'
  const isInProgress = progress?.status === 'in_progress'

  return (
    <div className={`bg-cyber-dark border rounded-lg overflow-hidden transition-all ${
      ...
      >
        <div className="flex items-center gap-3">
          {/* Confidence badge */}
          <span className={`text-[10px] px-2 py-1 rounded border ${CONFIDENCE_COLORS[hunt.confidence]}`}>
            ...
            <div className="px-4 py-3 border-b border-gray-800/50">
              ...
              <div className="space-y-3">
                {Object.entries(logQueries).map(([siemType, query]) => (
                  <div key={siemType} className="bg-gray-800/30 rounded-lg border border-gray-700 overflow-hidden">
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-1">
        <span className={`text-[10px] px-2 py-0.5 rounded border ${CONFIDENCE_COLORS[hunt.confidence]}`}>
          {hunt.confidence?.toUpperCase()}
        </span>
        {hunt.actor_name && (
          <span className="text-xs text-gray-500">{hunt.actor_name}</span>
        )}
      </div>
      <h4 className="text-sm text-white font-medium truncate">{hunt.title}</h4>
      <div className="flex items-center gap-2 mt-1">
        <span className="text-xs text-gray-500">
          {(hunt.quick_checks || []).length} checks
        </span>
