...
export function EventDetailPanel({ event, onClose }) {
  ...
  return (
    <div className="w-80 cyber-card flex-shrink-0 overflow-y-auto max-h-[calc(100vh-12rem)]">
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-4">
        <div className="flex-1 min-w-0">
          <EventTypeBadge type={event.event_type} size="sm" className="mb-2" />
          <h3 className="text-lg font-semibold text-white truncate" title={event.title}>
            {event.title}
          </h3>
          {event.subtitle && (
            <p className="text-sm text-gray-400 truncate">{event.subtitle}</p>
          )}
        </div>
        <button
          onClick={onClose}
