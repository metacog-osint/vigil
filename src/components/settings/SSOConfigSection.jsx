...
import { ssoConfig, SSO_PROVIDERS, generateSpMetadata, parseSamlMetadata, validateCertificate } from '../../lib/sso'
...
function CopyButton({ text, label }) {
  ...
  return (
    <button
      onClick={copy}
      className="text-xs text-cyber-accent hover:text-cyan-300"
    >
...
function MetadataDisplay({ metadata }) {
  return (
    <div className="bg-gray-800/50 rounded-lg p-4 space-y-3">
      ...
      <p className="text-xs text-gray-500">Use these values when configuring Vigil in your identity provider:</p>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href={metadata.metadataUrl} target="_blank" rel="noopener noreferrer" className="text-cyber-accent text-xs hover:underline">
...
-----END CERTIFICATE-----"
          ...
          {config.samlCertificate && (
            <p className={clsx(
              'text-xs mt-1',
              validateCertificate(config.samlCertificate).valid ? 'text-green-400' : 'text-red-400'
            )}>
              {validateCertificate(config.samlCertificate).valid ? '✓ Certificate format valid' : '✗ Invalid certificate format'}
            </p>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mb-2">Restrict SSO to specific email domains (leave empty to allow all)</p>
        ...
        {config.allowedDomains?.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {config.allowedDomains.map((domain) => (
              <span key={domain} className="flex items-center gap-1 px-2 py-1 bg-gray-800 rounded text-sm">
                {domain}
                <button onClick={() => removeDomain(domain)} className="text-gray-500 hover:text-red-400">×</button>
              </span>
            ))}
          </div>
        )}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-700">
        <div>
          ...
          <div className="text-xs text-gray-500">Require all users to sign in via SSO (password login disabled)</div>
        </div>
        <button
...
export default function SSOConfigSection() {
  ...
  if (!isAdmin) {
    return (
      <div className="text-gray-400 text-sm">
        Only tenant administrators can configure SSO.
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {config.isEnabled && (
        <div className={clsx(
          ...
          config.isEnforced ? 'bg-green-900/20 border border-green-800' : 'bg-blue-900/20 border border-blue-800'
        )}>
      ...
      {step === 'config' && config.provider && (
        <div className="space-y-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              {providerInfo?.docsUrl && (
                <a href={providerInfo.docsUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-cyber-accent hover:underline">
                  Setup documentation →
                </a>
              )}
            </div>
          ...
          <div className="border-t border-gray-700 pt-6">
            <h4 className="text-sm font-medium text-gray-300 mb-4">Identity Provider Configuration</h4>
          ...
          {testResult && (
            <div className={clsx(
              'p-4 rounded-lg',
              testResult.valid ? 'bg-green-900/20 border border-green-800' : 'bg-red-900/20 border border-red-800'
            )}>
              <div className={clsx('font-medium text-sm mb-2', testResult.valid ? 'text-green-400' : 'text-red-400')}>
