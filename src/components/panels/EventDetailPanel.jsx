...
export function EventDetailPanel({ event, onClose, isMobile = false }) {
  ...
  return (
    <div className={`${isMobile ? 'w-full' : 'w-80'} cyber-card flex-shrink-0 overflow-y-auto ${isMobile ? 'max-h-none' : 'max-h-[calc(100vh-12rem)]'}`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-4">
        <div className="flex-1 min-w-0">
          <EventTypeBadge type={event.event_type} size="sm" className="mb-2" />
          <h3 className={`font-semibold text-white ${isMobile ? 'text-base' : 'text-lg truncate'}`} title={event.title}>
            {event.title}
          </h3>
          {event.subtitle && (
            <p className={`text-sm text-gray-400 ${isMobile ? '' : 'truncate'}`}>{event.subtitle}</p>
          )}
        </div>
        {!isMobile && (
          <button
      ...
      <div className="space-y-4 text-sm">
        ...
        {event.event_type === 'ransomware' && (
          <RansomwareDetails event={event} />
        )}
        {event.event_type === 'alert' && (
          <AlertDetails event={event} />
        )}
        {event.event_type === 'vulnerability' && (
          <VulnerabilityDetails event={event} />
        )}
        {event.event_type === 'ioc' && (
          <IOCDetails event={event} />
        )}
        {event.event_type === 'malware' && (
          <MalwareDetails event={event} />
        )}
        {event.event_type === 'breach' && (
          <BreachDetails event={event} />
        )}
      ...
      {event.status && (
        <div>
          ...
          <span className={`px-2 py-0.5 rounded text-xs font-medium ${
            event.status === 'leaked' ? 'bg-red-500/20 text-red-400' :
            event.status === 'confirmed' ? 'bg-orange-500/20 text-orange-400' :
            event.status === 'claimed' ? 'bg-yellow-500/20 text-yellow-400' :
      ...
      {raw.ransom_amount && (
        <div>
          ...
          <div className="text-gray-300">
            {raw.ransom_currency || '$'}{Number(raw.ransom_amount).toLocaleString()}
          </div>
        </div>
      )}
    </>
      ...
      {event.pwn_count && (
        <div>
          ...
          <div className="text-2xl font-bold text-white">
            {event.pwn_count.toLocaleString()}
          </div>
        </div>
      )}
      {raw.domain && (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {raw.data_classes.slice(0, 6).map(dc => (
              <span key={dc} className="text-xs px-1.5 py-0.5 bg-purple-500/20 text-purple-400 rounded">
                {dc}
              </span>
            ))}
