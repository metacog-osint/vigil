  ...
  return (
    <div className="cyber-card">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/ransomware" className="text-xs text-cyan-400 hover:text-cyan-300">
          View all incidents →
        </Link>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {sectors.slice(0, 8).map((sector) => {
              ...
              const isUserSector =
                userSector && sector.name?.toLowerCase() === userSector.toLowerCase()
          ...
          {sector.recentIncidents?.length > 0 ? (
            <div className="space-y-2">
              {sector.recentIncidents.slice(0, 5).map((incident) => (
                <div key={incident.id} className="p-2 bg-gray-900/50 rounded">
