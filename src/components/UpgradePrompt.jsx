...
import {
  ...
  isLimitReached as isLimitReachedFn,
...
export function UpgradePrompt({
  ...
  if (variant === 'minimal') {
    return (
      <Link to="/pricing" className="text-cyan-400 hover:text-cyan-300 text-sm">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-400 text-sm mb-4">
            {featureDescription} is available on the {tierInfo?.name || requiredTier} plan and
            above.
          </p>
          <div className="flex items-center gap-4">
            ...
            {tierInfo?.price && (
              <span className="text-gray-500 text-sm">Starting at ${tierInfo.price}/month</span>
            )}
          </div>
        </div>
      </div>
...
export function ProBadge({ tier = 'professional' }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded ${colors[tier] || colors.professional}`}
    >
      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path
          strokeLinecap="round"
