...
export function ChangeSummaryCard({ data, loading }) {
  ...
  if (!data) {
    return <div className="cyber-card p-6 text-gray-400 text-center">No change data available</div>
  }
          ...
          <div className="space-y-1">
            {escalatingActors.slice(0, 5).map((actor) => (
              ...
              >
                ...
                <span className="text-xs text-red-400">{actor.incidents_7d || 0} incidents/7d</span>
              </Link>
            ))}
          </div>
        </div>
